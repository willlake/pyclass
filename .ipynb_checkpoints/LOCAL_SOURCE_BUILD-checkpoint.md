...
| `setup.py`, `build_class(...)` | Now dispatches on what `find_source` returned: a directory is packed, a file is copied, anything else is downloaded as before. The rest of the function — applying `pyclass/<branch>/patch`, then `make install` through `depends/Makefile` — is unchanged, so a local build goes through exactly the same path as a downloaded one. |
